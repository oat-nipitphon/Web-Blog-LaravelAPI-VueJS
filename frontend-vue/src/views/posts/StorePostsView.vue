<template>
  <div class="container bg-white shadow-lg rounded-lg mx-auto p-6">
    ...
    <div class="overflow-x-auto">
      <table class="w-full text-sm text-gray-700">
        <thead class="bg-gray-200 text-gray-700 uppercase">
          <tr>
            ...
            <th class="p-3 text-center">date store post</th>
        ...
        <tbody v-if="posts.length">
          ...
          >
            ...
            <td class="p-3 text-center">{{ post.posts.title }}</td>
            <td class="p-3 text-center text-gray-500">
              {{ formatDateTime(post.created_at) }}
            </td>
            <td class="p-3 text-center">
              <button
                @click="onRecoverPost(post.id)"
      ...
      <div
        class="flex justify-start mt-4 space-x-2"
        v-if="selectedItems.length"
      >
        <button
          class="bg-yellow-300 hover:bg-yellow-400 text-gray-900 text-sm font-medium py-2 px-4 rounded-lg shadow-md transition duration-300"
        ...
        <button
          class="bg-gray-500 hover:bg-gray-600 text-white text-sm font-medium py-2 px-4 rounded-lg shadow-md transition duration-300"
          @click="toggleAllCancel"
        >
          ยกเลิก
        </button>
...
const { storeGetPostStores, storeDeletePost, storeConfirmRecoverPost } =
  usePostStore();
...
profileID.value = authStore.users.userProfile.id;

onMounted(async () => {
  posts.value = await storeGetPostStores(route.params.profileID);
